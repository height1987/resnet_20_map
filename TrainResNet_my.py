...
def create_image_mb_source(map_file, mean_file, train, total_number_of_samples):
    ...
    if train:
        transforms += [
            xforms.crop(crop_type='randomside', side_ratio=(1.0, 1.0), jitter_type='uniratio') # train uses jitter
        ]
    transforms += [
...
if __name__=='__main__':
    ...
    parser.add_argument('-e', '--epochs', help='total epochs', type=int, required=False, default='90')
    parser.add_argument('-es', '--epoch_size', help='Size of epoch in samples', type=int, required=False, default='47015')
    ...
    reader_train = create_image_mb_source(os.path.join(data_path, 'train_map-2.txt'), os.path.join(data_path, 'mean.xml'), True, total_number_of_samples=epochs * epoch_size)
    reader_test = create_image_mb_source(os.path.join(data_path, 'test_map-2.txt'), os.path.join(data_path, 'mean.xml'), False, total_number_of_samples=C.io.FULL_DATA_SWEEP)
